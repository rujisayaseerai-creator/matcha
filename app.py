...
# ----------------- STEP 3 -----------------
    ...
    # โหลดไฟล์ qr code .jpeg
    try:
        st.image("qr_matcha.jpeg", caption="สแกนเพื่อชำระเงิน", use_column_width=True)
    except Exception:
        st.warning("⚠️ ไม่พบไฟล์ 'qr_matcha.jpeg' กรุณาตรวจสอบว่าอัปโหลดไฟล์ชื่อเดียวกันไว้ในโฟลเดอร์")

    st.markdown("### 🧾 แนบสลิปโอนเงิน")
    slip_file = st.file_uploader(
        "อัปโหลดสลิปโอนเงิน (ไฟล์ JPEG/PNG)",
        type=["png", "jpg", "jpeg"]
    )
    ...
    if confirm_btn:
        if slip_file is None:
            ...
            st.success("🎉 ขอบคุณค่ะ! ระบบได้รับออเดอร์และสลิปของคุณเรียบร้อยแล้ว")
            st.info("กรุณารอเรียกชื่อเมื่อเครื่องดื่มพร้อมเสิร์ฟนะคะ 💚🍵")
